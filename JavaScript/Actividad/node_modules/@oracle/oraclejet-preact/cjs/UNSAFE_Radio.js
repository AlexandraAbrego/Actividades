/* @oracle/oraclejet-preact: 15.1.3 */
'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var jsxRuntime = require('preact/jsx-runtime');
var hooks_UNSAFE_useComponentTheme = require('./hooks/UNSAFE_useComponentTheme.js');
var hooks_UNSAFE_useFocus = require('./hooks/UNSAFE_useFocus.js');
var hooks_UNSAFE_useFormContext = require('./hooks/UNSAFE_useFormContext.js');
var hooks_UNSAFE_useFormFieldContext = require('./hooks/UNSAFE_useFormFieldContext.js');
var hooks_UNSAFE_useId = require('./hooks/UNSAFE_useId.js');
var hooks_UNSAFE_useTabbableMode = require('./hooks/UNSAFE_useTabbableMode.js');
var hooks_UNSAFE_useInteractionStyle = require('./hooks/UNSAFE_useInteractionStyle.js');
var CheckboxRadioContext = require('./CheckboxRadioContext-69635e42.js');
var PRIVATE_ThemedIcons_CheckIcon = require('./PRIVATE_ThemedIcons/CheckIcon.js');
var PRIVATE_ThemedIcons_RadioOffIcon = require('./PRIVATE_ThemedIcons/RadioOffIcon.js');
var PRIVATE_ThemedIcons_RadioOnIcon = require('./PRIVATE_ThemedIcons/RadioOnIcon.js');
require('./UNSAFE_HiddenAccessible.js');
var RadioGroupContext = require('./RadioGroupContext-679d5bc0.js');
var UNSAFE_UserAssistance = require('./UNSAFE_UserAssistance.js');
require('./utils/UNSAFE_stringUtils.js');
var UNSAFE_Radio_themes_RadioIconContract_css = require('./UNSAFE_Radio/themes/RadioIconContract.css.js');
var UNSAFE_Radio_themes_redwood_RadioIconTheme = require('./UNSAFE_Radio/themes/redwood/RadioIconTheme.js');
var UNSAFE_Radio_themes_redwood_RadioTheme = require('./UNSAFE_Radio/themes/redwood/RadioTheme.js');
var HiddenAccessible = require('./HiddenAccessible-fdae5064.js');
var stringUtils = require('./stringUtils-ff6632eb.js');
require('./utils/UNSAFE_logger.js');
require('./_curry1-b2509ef1.js');
require('./_curry2-670aeaae.js');
require('./hooks/UNSAFE_useToggle.js');
require('preact/hooks');
require('preact');
require('./utils/UNSAFE_mergeProps.js');
require('./utils/PRIVATE_clientHints.js');
require('./clientHints-701bd447.js');
require('./hooks/UNSAFE_useHover.js');
require('./hooks/UNSAFE_useActive.js');
require('./UNSAFE_Icon.js');
require('./Icon-06e3b598.js');
require('./utils/UNSAFE_size.js');
require('./UNSAFE_Theme.js');
require('./utils-d94a578c.js');
require('./Common/themes/themeContract.css.js');
require('./hooks/UNSAFE_useTooltip.js');
require('./UNSAFE_Floating.js');
require('./Floating-47771a93.js');
require('preact/compat');
require('./index-76201516.js');
require('./hooks/UNSAFE_useUser.js');
require('./index-8629f35b.js');
require('./Common.js');
require('./Common/themes.js');
require('./Common/themes/redwood/theme.js');
require('./utils/PRIVATE_floatingUtils.js');
require('./utils/PRIVATE_refUtils.js');
require('./hooks/UNSAFE_useOutsideClick.js');
require('./utils/UNSAFE_arrayUtils.js');
require('./utils/UNSAFE_classNames.js');
require('./classNames-58437073.js');
require('./UNSAFE_Floating/themes/redwood/FloatingTheme.js');
require('./UNSAFE_Floating/themes/FloatingStyles.css.js');

require('./UNSAFE_Floating/themes/redwood/FloatingBaseTheme.css.js');

require('./UNSAFE_Floating/themes/redwood/FloatingVariants.css.js');
require('./vanilla-extract-recipes-createRuntimeFn.esm-9376b7de.js');
require('./vanilla-extract-dynamic.esm-20a05a42.js');
require('./UNSAFE_Floating/themes/FloatingContract.css.js');
require('./UNSAFE_Layer.js');
require('./useThemeInterpolations-160b079d.js');
require('./hooks/UNSAFE_useColorScheme.js');
require('./hooks/UNSAFE_useScale.js');
require('./utils/UNSAFE_interpolations/theme.js');
require('./utils/UNSAFE_mergeInterpolations.js');
require('./_curry3-4effaded.js');
require('./_has-a8e6bd8f.js');
require('./hooks/UNSAFE_useTouch.js');
require('./hooks/UNSAFE_useAnimation.js');
require('./useAnimation-6b3f82d5.js');
require('./hooks/UNSAFE_useThemeInterpolations.js');
require('./hooks/UNSAFE_useTooltip/themes/redwood/TooltipContentTheme.js');
require('./hooks/UNSAFE_useTooltip/themes/TooltipContentStyles.css.js');

require('./hooks/UNSAFE_useTooltip/themes/redwood/TooltipContentBaseTheme.css.js');

require('./hooks/UNSAFE_useTooltip/themes/redwood/TooltipContentVariants.css.js');

require('./UNSAFE_Icon/themes/IconStyle.css.js');


require('./InlineHelpSource-126e9455.js');
require('./hooks/UNSAFE_useTranslationBundle.js');
require('./UNSAFE_UserAssistance/themes/redwood/UserAssistanceTheme.js');
require('./UNSAFE_UserAssistance/themes/UserAssistanceStyles.css.js');

require('./UNSAFE_UserAssistance/themes/redwood/UserAssistanceBaseTheme.css.js');

require('./UNSAFE_UserAssistance/themes/redwood/UserAssistanceVariants.css.js');
require('./UNSAFE_ComponentMessage.js');
require('./ComponentMessage-898c0a60.js');
require('./PRIVATE_Message.js');
require('./UNSAFE_Flex.js');
require('./Flex-ca3ece5b.js');
require('./utils/UNSAFE_interpolations/dimensions.js');
require('./utils/UNSAFE_interpolations/boxalignment.js');
require('./keys-ea4d3d4e.js');
require('./utils/UNSAFE_interpolations/flexbox.js');
require('./flexbox-4ee1d134.js');
require('./utils/UNSAFE_interpolations/flexitem.js');
require('./flexitem-dcfe9df9.js');
require('./utils/PRIVATE_timer.js');
require('./PRIVATE_Message/themes/MessageStyles.css.js');

require('./MessageCloseButton-4b47e7d2.js');
require('./UNSAFE_IconButton.js');
require('./IconButton-4fcef211.js');
require('./UNSAFE_BaseButton.js');
require('./BaseButton-ce8c5c12.js');
require('./hooks/UNSAFE_usePress.js');
require('./UNSAFE_ButtonLayout.js');
require('./ButtonLayout-ffa5087b.js');
require('./UNSAFE_Text.js');
require('./Text-e57cfc67.js');
require('./UNSAFE_Text/themes/TextStyles.css.js');

require('./PRIVATE_ThemedIcons/CloseIcon.js');
require('./MessageDetail-4596ae8a.js');
require('./MessageFormattingUtils-70bf5f34.js');
require('./utils/UNSAFE_getLocale.js');
require('./Message.types-aa907644.js');
require('./MessageStartIcon-aa131bab.js');
require('./PRIVATE_ThemedIcons/MessageConfirmationIcon.js');
require('./PRIVATE_ThemedIcons/MessageErrorIcon.js');
require('./PRIVATE_ThemedIcons/MessageInfoIcon.js');
require('./PRIVATE_ThemedIcons/MessageWarningIcon.js');
require('./MessageSummary-491ce9bc.js');
require('./MessageTimestamp-3759c923.js');
require('./MessageUtils-d7e23e38.js');
require('./utils/UNSAFE_soundUtils.js');
require('./UNSAFE_MessageBanner/themes/redwood/MessageBannerTheme.js');
require('./UNSAFE_MessageBanner/themes/MessageBannerStyles.css.js');

require('./UNSAFE_MessageBanner/themes/redwood/MessageBannerBaseTheme.css.js');

require('./UNSAFE_MessageBanner/themes/redwood/MessageBannerVariants.css.js');

require('./MessagesManager-1af82e68.js');
require('./PRIVATE_TransitionGroup.js');
require('./hooks/UNSAFE_useMessagesContext.js');
require('./UNSAFE_ComponentMessage/themes/redwood/ComponentMessageTheme.js');
require('./UNSAFE_ComponentMessage/themes/ComponentMessageStyles.css.js');

require('./UNSAFE_ComponentMessage/themes/redwood/ComponentMessageBaseTheme.css.js');

require('./UNSAFE_ComponentMessage/themes/redwood/ComponentMessageVariants.css.js');
require('./ComponentMessageContainer-64d3846e.js');
require('./hooks/UNSAFE_useInputGroupContext.js');
require('./UNSAFE_Popup.js');
require('./utils/PRIVATE_tabbableUtils.js');
require('./_arity-7154888d.js');
require('./UNSAFE_FocusTrap.js');
require('./FocusTrap-904118b1.js');
require('./hooks/PRIVATE_useAnimationStatus.js');
require('./UNSAFE_Popup/themes/PopupContract.css.js');
require('./UNSAFE_Popup/themes/redwood/PopupTheme.js');
require('./UNSAFE_Popup/themes/redwood/PopupBaseTheme.css.js');

require('./UNSAFE_Popup/themes/redwood/PopupVariants.css.js');

require('./UNSAFE_Divider.js');
require('./Divider-86cd19e5.js');
require('./UNSAFE_Divider/themes/DividerStyles.css.js');

require('./CompactHelpSource-cb2b421b.js');
require('./PRIVATE_ThemedIcons/HelpIcon.js');
require('./hooks/UNSAFE_useFocusWithin.js');
require('./useFocusWithin-1f0a2cd6.js');
require('./UNSAFE_Label/themes/redwood/LabelTheme.js');
require('./UNSAFE_Label/themes/LabelStyles.css.js');

require('./UNSAFE_Label/themes/redwood/LabelBaseTheme.css.js');

require('./UNSAFE_Label/themes/redwood/LabelVariants.css.js');
require('./UNSAFE_Radio/themes/RadioIconStyles.css.js');

require('./UNSAFE_Radio/themes/redwood/RadioIconBaseTheme.css.js');

require('./UNSAFE_Radio/themes/redwood/RadioIconVariants.css.js');

require('./UNSAFE_Radio/themes/RadioStyles.css.js');

require('./UNSAFE_Radio/themes/redwood/RadioBaseTheme.css.js');

require('./UNSAFE_Radio/themes/redwood/RadioVariants.css.js');

const Radio = ({ value, children, ...assistanceProps }) => {
    const { isReadonly: isFormReadonly, isDisabled: isFormDisabled } = hooks_UNSAFE_useFormContext.useFormContext();
    const { isDisabled: isFieldDisabled, isReadonly: isFieldReadonly } = hooks_UNSAFE_useFormFieldContext.useFormFieldContext();
    const { name, value: groupValue, onCommit } = RadioGroupContext.useRadioGroupContext();
    const { isFocusRingShown } = CheckboxRadioContext.useCheckboxRadioContext();
    const { isTabbable } = hooks_UNSAFE_useTabbableMode.useTabbableMode();
    const id = hooks_UNSAFE_useId.useId();
    const { isFocus, focusProps } = hooks_UNSAFE_useFocus.useFocus();
    const isSelected = groupValue === value;
    const isDisabled = isFieldDisabled ?? isFormDisabled;
    const isReadonly = isFieldReadonly ?? isFormReadonly;
    const { interactionProps, applyActiveStyle } = hooks_UNSAFE_useInteractionStyle.useInteractionStyle();
    const { classes: radioClasses, styles: { valueLabelStyle, valueTextStyle } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_Radio_themes_redwood_RadioTheme.RadioRedwoodTheme, {
        disabled: isDisabled ? 'isDisabled' : 'notDisabled',
        focused: isFocus && isFocusRingShown ? 'isFocused' : ''
    });
    const { classes: radioIconClasses } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_Radio_themes_redwood_RadioIconTheme.RadioIconRedwoodTheme, {
        disabled: isDisabled ? 'isDisabled' : 'notDisabled',
        selected: isSelected ? 'isSelected' : 'notSelected',
        readonly: isReadonly ? 'isReadonly' : 'notReadonly',
        active: applyActiveStyle ? 'isActive' : 'notActive'
    });
    const { assistiveText, helpSourceLink } = assistanceProps;
    const hasUserAssistive = !isReadonly && !!(assistiveText || helpSourceLink);
    const uaId = hasUserAssistive ? `${id}-icon-ua` : undefined;
    const IconComp = isReadonly
        ? isSelected
            ? PRIVATE_ThemedIcons_CheckIcon.CheckIcon
            : PRIVATE_ThemedIcons_RadioOffIcon.RadioOffIcon
        : isSelected
            ? PRIVATE_ThemedIcons_RadioOnIcon.RadioOnIcon
            : PRIVATE_ThemedIcons_RadioOffIcon.RadioOffIcon;
    return (jsxRuntime.jsx("div", { children: jsxRuntime.jsxs("label", { class: radioClasses, ...interactionProps, children: [jsxRuntime.jsx(HiddenAccessible.HiddenAccessible, { children: jsxRuntime.jsx("input", { type: "radio", name: name, id: id, value: value, ...(!isReadonly && {
                            onChange: (e) => {
                                e.stopPropagation();
                                onCommit?.({ previousValue: groupValue, value });
                            }
                        }), checked: isSelected, disabled: isDisabled, "aria-describedby": uaId, ...(!isTabbable && { tabIndex: -1 }), ...(isReadonly && { 'aria-readonly': true }), ...focusProps }) }), jsxRuntime.jsx("span", { "aria-hidden": "true", class: radioIconClasses, children: jsxRuntime.jsx(IconComp, { size: stringUtils.getVarName(UNSAFE_Radio_themes_RadioIconContract_css.radioIconVars.iconSize), color: "currentColor" }) }), jsxRuntime.jsxs("span", { class: valueLabelStyle, children: [jsxRuntime.jsx("span", { class: valueTextStyle, children: children }), hasUserAssistive && (jsxRuntime.jsx(UNSAFE_UserAssistance.IconUserAssistance, { ...assistanceProps, isTabbable: isTabbable && isSelected && isFocus, id: uaId }))] })] }) }));
};

exports.Radio = Radio;
//# sourceMappingURL=UNSAFE_Radio.js.map
